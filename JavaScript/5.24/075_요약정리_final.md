# JavaScript
* HTML은 컨텐츠와 골격, CSS는 웹페이지의 레이아웃,
JavaScript는 웹페이지의 동작,
웹어셈블리(WASM, JS의 최적화)가 추가되면서 웹의 4대 요소 HTML,CSS,JS,Web Assembly
    * ES1 (초판, 1997)
    * ES2 (1998)
    * ES3 (1999)
...
## 코드 구조
1. 문(statement)은 세미콜론으로 구분
2. 문은  값, 연산자, 키워드, 명령어, 표현식(값으로 평가, 함수나 key, index를 통한 값의 호출도 표현식) 등으로 구성됩니다.
3. 공백 병합
- 성능(실행속도)에는 차이없는데, 로딩속도에는 차이가 있어서 js compress 사용해서 공백 줄여줌
  ```js
  let x = 10,
      y = 20,
      z = 30
...
* var, let, const
    ...
    * let(ES5) : 블록 레벨 스코프 (블록 레벨 = 중괄호), 재선언시 애러 O, 콘솔에서는 애러 X, 변경가능한 자료형
    * const(ES5) : 블록 레벨 스코프 (블록 레벨 = 중괄호), 재선언시 애러 O, 콘솔에서는 애러 X, 변경이 불가능한 자료형(상수)
...
3. 논리 연산자(&&, ||, !, !!, &, |)
    ...
    * 암기코드
    ...
    ```
    ```js
    // 앞에 값이 null인지를 확인하고 싶은 경우, 단락 회로 평가라고 부릅니다.
    result1 = 10 || 100;
    result2 = 0 && 100;
    result3 = null || 100;
    result4 = null && 100;
...
- Number(숫자)
    ...
    * 호출 : 변수명
    * 메서드 : 
        * 10.toString()는 안됩니다. 이유는 무엇일까요? 소수점 때문에 그렇습니다.(JavaScript의 parsing때문)
        * (10).toString()와 변수명.toString()은 가능합니다.
        * num.toFixed(): Math.round와 유사하게 가장 가까운 값으로 올림 혹은 버림, 메서드 반환 값이 문자열인 점 주의!
    * Number()
    * parseInt() & parseFloat()
      - 두 함수는 불가능할 때까지 문자열에서 숫자를 '읽습니다'.
      - parseInt는 정수, parseFloat는 부동 소수점 숫자를 반환
      - parseInt()의 두 번째 매개 변수는 선택적으로 사용하여 원하는 진수를 지정
      ```js
    ...
    * Math
        ...
        * Math.max(), Math.min() : 인수 중 최대/최솟값을 반환
        ```js
        Math.max([1, 2, 3, 4, 10, 2, 3]) // NaN
        Math.max(...[1, 2, 3, 4, 10, 2, 3]) // 10
        ```
        ...
        * Math.trunc() : 소수부를 무시. 3.1은 3이 되고 -1.1은 -1이 됩니다. (Internet Explorer에서는 지원하지 않음)
    ...
    * Infinity, -Infinity (typeof Infinity, -Infinity = 'number')

- String(문자열, 열=순서가있다)
    * 형태 : 'abcde', "abcde", `abcde ${변수명}`
    * 호출 : 변수명, 변수명[0] (변수명[index]를 통해 호출은 할 수 있지만 개별 값 변경 불가 = immutable)
    ```js
    let x = 'abcd'
    x[0] = 'c'
    console.log(x) // 'abcd'
    ...
    * 메서드 : 
        * str.length
        * str.indexOf() : 주어진 값과 일치하는 첫 번째 인덱스를 반환
        * str.lastIndexOf()
        * str.includes() : 하나의 문자열이 다른 문자열에 포함되어 있는지를 판별하고, 결과를 true 또는 false 로 반환
        * str.slice(beginIndex, endIndex) : 시작점~ 종료점 인덱스 직전까지 추출
        * str.splice() :  기존 요소를 삭제 또는 교체하거나 새 요소를 추가
        * str.split() :주어진 문자열을 separator마다 끊은 부분 문자열을 담은 Array 반환
        * str.substring() : 시작 인덱스로 부터 종료 인덱스 전 까지 문자열의 부분 문자열을 반환
        * str.toLowerCase()
        * str.toUpperCase()
        * str.trim() : 문자열 양 끝의 공백을 제거
        * str.replace() : 일치하는 일부 또는 모든 부분이 교체된 새로운 문자열을 반환
        * str.concat() : 매개변수로 전달된 모든 문자열을 호출 문자열에 붙인 새로운 문자열을 반환
        * str.repeat() : 문자열을 주어진 횟수만큼 반복해 붙인 새로운 문자열을 반환
          ```js
          'hello'.repeat(100)
          '0'.repeat(100)
...
- Boolean(논리값)
    * 형태 : true, false
    * 호출 : 변수명
    * 어떤 것이 true이고 어떤 것이 false인지 판단할 수 있어야 합니다.(truthy, falsy -> 우리가 매우 깊이 다뤘던 내용입니다.) => !! 로 확인
...
- Array(배열) : object
    ...
    * 호출 : 변수명, 변수명[0], 변수명[0][0] (변수명[index], 개별값 변경 가능 = mutable)
    * Array
    * 메서드 (클래스 내의 함수) : 
        * length
        * forEach
        * map : 데이터를 로드해오고 새로운 배열로 반환
          ```js
          [{name: 'hojun1', age: 10},
           {name: 'hojun1', age: 10},
           {name: 'hojun1', age: 10}
